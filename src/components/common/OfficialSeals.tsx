...
export function useOfficialLogos() {
  const getLogoMap = () => {
    ...
    if (config.systemLogoUrl && config.systemLogoUrl !== '/icon.jpg') map['logo-system'] = config.systemLogoUrl;
    if (config.headerLogoUrl && config.headerLogoUrl !== '/icon.jpg') map['logo-header'] = config.headerLogoUrl;
    if (config.footerLogoUrl && config.footerLogoUrl !== '/icon.jpg') map['logo-footer'] = config.footerLogoUrl;
    if (config.websiteLogoUrl && config.websiteLogoUrl !== '/icon.jpg') map['logo-website'] = config.websiteLogoUrl;
...
export const SealDA: React.FC<{ className?: string; customUrl?: string }> = ({
  ...
  if (activeUrl && activeUrl.trim() !== '' && activeUrl !== '/icon.jpg') {
    return (
      <img
        src={activeUrl}
...
export const DynamicSeal: React.FC<{
  ...
  if (normalized === 'sidebar' || normalized === 'system' || normalized === 'header' || normalized === 'footer' || normalized === 'login') {
    ...
    if (custom && custom.trim() !== '' && custom !== '/icon.jpg') {
      return (
        <img
          src={custom}
          alt={`${type} Logo`}
  ...
  if (fallbackUrl && fallbackUrl.trim() !== '' && fallbackUrl !== '/icon.jpg') {
    return (
      <img
        src={fallbackUrl}
        alt={`${type} Logo`}
