...
export const CertificateManager: React.FC<CertificateManagerProps> = ({
  ...
  const [leftLogoUrl, setLeftLogoUrl] = useState(config.daLogoUrl || '/icon.jpg');
  const [centerLogoUrl, setCenterLogoUrl] = useState(config.centerLogoUrl || '/icon.jpg');
  const [rightLogoUrl, setRightLogoUrl] = useState(config.lguLogoUrl || '/icon.jpg');
  const [watermarkLogoUrl, setWatermarkLogoUrl] = useState(config.watermarkLogoUrl || config.lguLogoUrl || '/icon.jpg');
        ...
        <div className="bg-amber-50/80 border border-amber-200 rounded-2xl p-4 shadow-sm space-y-3 animate-fadeIn text-xs">
          ...
          <div className="flex items-center justify-between pt-1">
            <button
              type="button"
              onClick={() => {
                setLeftLogoUrl('/icon.jpg');
                setCenterLogoUrl('/icon.jpg');
                setRightLogoUrl('/icon.jpg');
              }}
