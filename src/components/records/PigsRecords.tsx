...
export const PigsRecords: React.FC<PigsRecordsProps> = ({
  ...
  const [reportTitle, setReportTitle] = useState('Official Swine Registry Report');
  const [reportLandscape, setReportLandscape] = useState(true);
  ...
  const [reportLeftLogo, setReportLeftLogo] = useState<string>('/icon.jpg');
  const [reportRightLogo, setReportRightLogo] = useState<string>('/icon.jpg');
  ...
  const toggleReportColumn = (column: keyof typeof reportColumns) => {
    setReportColumns(previous => ({ ...previous, [column]: !previous[column] }));
  };

  const reportColumnDefinitions = [
    ['earTag', 'Ear tag'], ['farmer', 'Farmer'], ['barangay', 'Barangay'], ['breed', 'Breed / type'],
    ['weight', 'Weight'], ['price', 'Est. price'], ['status', 'Status'], ['contact', 'Contact'],
    ['rsbsa', 'RSBSA ID'], ['coordinates', 'Coordinates'],
  ] as const;
          ...
          <div className="bg-white w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden my-6 border border-stone-200">
            ...
            <div className="bg-stone-900 text-white px-6 py-4 flex items-center justify-between flex-wrap gap-2">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-bold text-sm">Print Official Registry Report</h3>
                  <p className="text-[10px] text-stone-400">Choose the fields and format for a clean municipal report</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button
            ...
            <div className="no-print grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-4 p-4 bg-stone-50 border-b border-stone-200">
              <div>
                <label className="block text-[11px] font-bold uppercase tracking-wide text-stone-500 mb-1">Report title</label>
                <input
                  value={reportTitle}
                  onChange={e => setReportTitle(e.target.value)}
                  className="w-full rounded-lg border border-stone-300 bg-white px-3 py-2 text-xs text-stone-800 outline-none focus:border-emerald-500"
                />
              </div>
              <label className="flex items-center gap-2 self-end pb-2 text-xs font-semibold text-stone-700 cursor-pointer">
                <input type="checkbox" checked={reportLandscape} onChange={e => setReportLandscape(e.target.checked)} className="accent-emerald-600" />
                Landscape layout
              </label>
              <div className="lg:col-span-2">
                <span className="block text-[11px] font-bold uppercase tracking-wide text-stone-500 mb-2">Include data fields</span>
                <div className="flex flex-wrap gap-2">
                  {reportColumnDefinitions.map(([key, label]) => (
                    <label key={key} className={`inline-flex items-center gap-1.5 rounded-lg border px-2.5 py-1.5 text-[11px] font-semibold cursor-pointer ${reportColumns[key] ? 'border-emerald-300 bg-emerald-50 text-emerald-800' : 'border-stone-300 bg-white text-stone-500'}`}>
                      <input type="checkbox" checked={reportColumns[key]} onChange={() => toggleReportColumn(key)} className="accent-emerald-600" />
                      {label}
                    </label>
                  ))}
                </div>
              ...
              <div className="bg-amber-50/70 border-b border-amber-200 p-4 space-y-3 animate-fadeIn">
                ...
                <div className="flex justify-end gap-2 pt-1">
                  <button
                    type="button"
                    onClick={() => {
                      setReportLeftLogo('/icon.jpg');
                      setReportRightLogo('/icon.jpg');
                    }}
            ...
            <div className={`p-8 space-y-6 text-xs text-stone-800 printable-document bg-white ${reportLandscape ? 'report-landscape' : ''}`}>
              {/* Official Header */}
              ...
              <table className="w-full text-left border border-stone-300 text-[11px]">
                <thead className="bg-stone-100 font-bold text-stone-700 border-b border-stone-300">
                  <tr>
                    ...
                    {reportColumns.earTag && <th className="p-2 border-r border-stone-300">Ear Tag</th>}
                    {reportColumns.farmer && <th className="p-2 border-r border-stone-300">Farmer Name</th>}
                    {reportColumns.barangay && <th className="p-2 border-r border-stone-300">Barangay</th>}
                    {reportColumns.breed && <th className="p-2 border-r border-stone-300">Breed / Type</th>}
                    {reportColumns.weight && <th className="p-2 border-r border-stone-300 text-right">Weight</th>}
                    {reportColumns.price && <th className="p-2 border-r border-stone-300 text-right">Est. Price</th>}
                    {reportColumns.status && <th className="p-2 border-r border-stone-300 text-center">Status</th>}
                    {reportColumns.contact && <th className="p-2 border-r border-stone-300">Contact</th>}
                    {reportColumns.rsbsa && <th className="p-2 border-r border-stone-300">RSBSA ID</th>}
                    {reportColumns.coordinates && <th className="p-2 text-right">Coordinates</th>}
                  </tr>
                </thead>
                <tbody className="divide-y divide-stone-300">
                  {filtered.map((s, idx) => (
                    <tr key={s.id}>
                      ...
                      {reportColumns.earTag && <td className="p-2 border-r border-stone-300 font-mono font-bold text-emerald-900">{s.earTagNo}</td>}
                      {reportColumns.farmer && <td className="p-2 border-r border-stone-300 font-semibold">{s.farmerName}</td>}
                      {reportColumns.barangay && <td className="p-2 border-r border-stone-300">Brgy. {s.barangay}</td>}
                      {reportColumns.breed && <td className="p-2 border-r border-stone-300">{s.breed} ({s.swineType})</td>}
                      {reportColumns.weight && <td className="p-2 border-r border-stone-300 text-right">{s.weightKg} kg</td>}
                      {reportColumns.price && <td className="p-2 border-r border-stone-300 text-right">₱{(s.estimatedPricePhp || 0).toLocaleString()}</td>}
                      {reportColumns.status && <td className="p-2 border-r border-stone-300 text-center font-bold text-[10px]">
                        {s.readyToSell ? 'READY TO SELL' : s.status.toUpperCase()}
                      </td>}
                      {reportColumns.contact && <td className="p-2 border-r border-stone-300">{s.farmerContact}</td>}
                      {reportColumns.rsbsa && <td className="p-2 border-r border-stone-300">{s.rsbsaId || '—'}</td>}
                      {reportColumns.coordinates && <td className="p-2 text-right">{s.latitude.toFixed(5)}, {s.longitude.toFixed(5)}</td>}
                    </tr>
                  ))}
                </tbody>
              </table>
