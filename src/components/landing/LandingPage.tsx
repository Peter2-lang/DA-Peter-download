...
export const LandingPage: React.FC<LandingPageProps> = ({
  ...
  const activeSwine = effectiveSwineList.filter(s => !s.isArchived);
  const totalSwine = activeSwine.length;
  const uniqueFarmers = new Set(activeSwine.map(s => s.farmerName.trim()).filter(Boolean)).size;
  const biosecurityAudits = storageService.getBiosecurityAudits();
  const complianceRate = biosecurityAudits.length
    ? biosecurityAudits.reduce((total, audit) => total + audit.complianceScore, 0) / biosecurityAudits.length
    : 0;

  const liveStatValues: Record<string, string> = {
    'stat-1': `${effectiveBarangays.length}`,
    'stat-2': `${uniqueFarmers}`,
    'stat-3': `${totalSwine}`,
    'stat-4': `${complianceRate.toFixed(1)}%`,
  };
                ...
                cmsConfig.stats
                  ...
                  .map(stat => (
                    ...
                    >
                      ...
                      >
                        {liveStatValues[stat.id] ?? stat.value}
                      </span>
                      {stat.description && (
