...
function normalizeBranding(config: LandingCmsConfig): LandingCmsConfig {
  const branding = { ...config };
  if (branding.faviconUrl === '/icon.svg') branding.faviconUrl = '/icon.jpg';
  if (branding.systemLogoUrl === '/icon.svg') branding.systemLogoUrl = '/icon.jpg';
  if (branding.headerLogoUrl === '/icon.svg') branding.headerLogoUrl = '/icon.jpg';
  if (branding.footerLogoUrl === '/icon.svg') branding.footerLogoUrl = '/icon.jpg';
  return branding;
}
...
export const landingCmsService = {
  getPublishedConfig(): LandingCmsConfig {
    ...
    if (normalized !== data) setLocalItem(CMS_STORAGE_KEYS.PUBLISHED, normalized);
    return normalized;
  },

  getDraftConfig(): LandingCmsConfig {
    const draft = getLocalItem<LandingCmsConfig | null>(CMS_STORAGE_KEYS.DRAFT, null);
    if (draft) return normalizeBranding(draft);
